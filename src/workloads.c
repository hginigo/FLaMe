...
{
    struct task *aux;
    struct block *aux_block;
    struct link *l;
    int count = 0;
    struct vp_vec *adj_list = &t->adj_lists[n->id];

    aux_block = vp_vec_get(bl, n->id);
    aux = task_alloc(n, aux_block, TRAIN);
    ...
    vp_for (l, adj_list) {
        ...
        aux_block = vp_vec_get(bl, l->dest);
        aux = task_alloc(n, aux_block, READ);
        vp_vec_append(tl, aux);
    }
    aux_block = vp_vec_get(bl, n->id);
    aux = task_alloc(n, aux_block, WRITE);
    aux->size = 0;
    vp_vec_append(tl, aux);
    if (config.barriers) {
